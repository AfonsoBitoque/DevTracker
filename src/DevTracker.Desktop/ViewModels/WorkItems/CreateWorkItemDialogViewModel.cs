...
public sealed partial class CreateWorkItemDialogViewModel(
    IWorkItemService workItemService) : DialogViewModelBase
{
    ...
    private async Task ConfirmAsync()
    {
        ...
        try
        {
            var result = await workItemService.CreateAsync(new CreateWorkItemRequest(
                ProjectId,
                Title.Trim(),
        ...
        catch (AuthorizationException) { SetError("No permission to create work items."); }
